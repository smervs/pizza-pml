    ...
    <div class="m-10 flex w-full flex-col sm:flex-row">
        ...
        <form method="POST" action="/orders" class="flex-1">
            <div class="flex flex-col w-full md:w-3/6">
                ...
                <textarea name="order" required rows="20" class="p-2 border border-gray-500 rounded-md w-full">{{ old('order') }}</textarea>
                @if ($errors)
            ...
            <button type="submit" class="ml-1 bg-red-500 text-white px-6 py-2 rounded-full mt-4 shadow-xl">Save</button>
        </form>
        {{-- /Form --}}
    </div>
@endsection